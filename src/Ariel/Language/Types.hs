...
instance SexpIso Decl where
    sexpIso = match
        ...
        $ With (list (el (sym "define") >>> el symbol >>> el (list (rest symbol)) >>> el sexpIso) >>>)
        End
...
instance SexpIso Expr where
    sexpIso = match
        ...
        $ With (list (el sexpIso >>> rest sexpIso) >>>)
        End

data Pattern = Pattern0 Text
             | Pattern Text [Text]
    deriving (Eq, Show, Generic)
...
instance SexpIso Pattern where
    sexpIso = match
        $ With (quoted symbol >>>)
        $ With (list (el (quoted symbol) >>> rest symbol) >>>)
        End
