...
main(int argc, char *argv[])
{
    ...
    if ( !PL_set_resource_db_mem(ariel_state, ariel_state_len) ||
       !PL_initialise(argc, swipl_argv) ) {
        PL_halt(1);
    }
    ...
    while(1) {
        ...
        if(buffer == NULL) {
            ...
            PL_put_string_chars(ariel_str_arg, buffer);
            qid_t q = PL_open_query(module_api, PL_Q_NORMAL, pred_run_query, query_args);
            PL_next_solution(q);
            PL_get_chars(query_res_arg, &response, CVT_STRING | BUF_DISCARDABLE);
            printf("%s\n", response);
            PL_close_query(q);
            add_history(buffer);
            free(buffer);
